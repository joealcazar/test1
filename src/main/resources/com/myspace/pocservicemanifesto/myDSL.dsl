...
[when]The difference between the created date time and the requested date time is {age} days and the shipping condition selected for the Order is "{message}"=order:Order( java.time.OffsetDateTime.parse(requestedDateStr).compareTo(java.time.OffsetDateTime.parse(orderDateStr))=={age} && shippingCondition.equalsIgnoreCase("{message}"))
[when]The difference between the created date time and the requested date time is {age} days, the shipping condition selected for the Order is "{message}" and the distance between JobSite and Plant is more than {distance} = order:Order( requestedDate.compareTo(orderDate)=={age} && shippingCondition.equalsIgnoreCase("{message}") && distanceJobSite_Plant > {distance})
