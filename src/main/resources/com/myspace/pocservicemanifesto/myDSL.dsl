...
[when]The difference between the created date time and the requested date time is {age} days and the shipping condition selected for the Order is "{message}"=order:Order((java.time.OffsetDateTime.parse(requestedDateStr).comparesTo(java.time.OffsetDateTime.parse(orderDateStr))=={age}) && (shippingCondition.equalsIgnoreCase("{message}")))
